defmodule Parrot.Media.AudioChunkerIntegrationTest do
  ...
  describe "OPUS codec compatibility" do
    test "chunks raw audio for OPUS encoder (48kHz, 960 samples)" do
      ...
      {[], state} = AudioChunker.handle_init(nil, %{
        chunk_samples: 960,  # OPUS needs exactly 960 samples at 48kHz for 20ms
      ...
      buffer_sizes = [512, 1024, 753, 1200, 800]  # Random sizes from PortAudio
  ...
  describe "PCMA codec compatibility" do
    test "chunks G.711 for PCMA RTP packets (8kHz, 20ms)" do
      ...
      {[], state} = AudioChunker.handle_init(nil, %{
        chunk_samples: nil,
        chunk_duration_ms: 20,  # Standard RTP packet duration
        sample_rate: 8000       # G.711 operates at 8kHz
      })
      ...
      buffer_sizes = [100, 200, 150, 180, 90]  # Random sizes
      
      chunks = 
        Enum.reduce(buffer_sizes, {[], state}, fn size, {chunks_acc, state_acc} ->
          buffer = %Buffer{
            payload: :binary.copy(<<0xD5>>, size),  # G.711 silence pattern
            pts: nil
          }
          {[buffer: {:output, new_chunks}], new_state} = 
            AudioChunker.handle_buffer(:input, buffer, nil, state_acc)
  ...
  describe "codec switching scenario" do
    test "can be reconfigured between OPUS and PCMA requirements" do
      ...
      {[], opus_state} = AudioChunker.handle_init(nil, %{
        chunk_samples: 960,
        chunk_duration_ms: nil,
        sample_rate: nil
      })
      ...
      assert opus_state.bytes_per_chunk == 1920  # 960 samples * 2 bytes
      
      # Now configure a new instance for PCMA
      {[], pcma_state} = AudioChunker.handle_init(nil, %{
        chunk_samples: nil,
      ...
      assert pcma_state.bytes_per_chunk == 160  # 160 samples * 1 byte
