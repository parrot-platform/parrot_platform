defmodule Parrot.Media.SimpleRTPReceiver do
  ...
  def handle_stream_format(:input, _stream_format, _ctx, state) do
    ...
    format = case state.selected_codec do
  ...
  def handle_buffer(:input, buffer, _ctx, state) do
    ...
    case parse_rtp_packet(buffer.payload) do
      {:ok, payload, seq_num, timestamp, payload_type} ->
        Logger.debug("RTP packet received: seq=#{seq_num}, ts=#{timestamp}, pt=#{payload_type}, payload_size=#{byte_size(payload)}")
      ...
      {:error, reason} ->
        Logger.warning("Failed to parse RTP packet: #{reason}, buffer size: #{byte_size(buffer.payload)}")
  ...
  defp parse_rtp_packet(<<
    _version::2, _padding::1, _extension::1, cc::4,
    _marker::1, payload_type::7,
    seq_num::16,
    timestamp::32,
    _ssrc::32,
